...
from .serializers import (
    UserSerializer, LoginSerializer, StudentSerializer, FacultySerializer,
    AdminSerializer, StudentProfileUpdateSerializer, FacultyProfileUpdateSerializer,
    CreateUserSerializer, PasswordResetSerializer,
)
from django.core.mail import send_mail
from django.conf import settings
from academics.models import Course, Subject, SemesterResult
from academics.serializers import SemesterResultSerializer, CourseSerializer, SubjectSerializer
...
def student_dashboard(request):
    ...
    return Response({
        ...
        'latest_sgpa': float(latest_result.sgpa) if latest_result and latest_result.sgpa else None,
...
def student_submit_assignment(request):
    ...
    return Response({'error': 'Assignment submission is disabled'}, status=status.HTTP_400_BAD_REQUEST)
...
def faculty_dashboard(request):
    ...
    total_students = Student.objects.filter(course_id__in=course_ids).count() if course_ids else 0
...
def faculty_students(request):
    ...
    students = Student.objects.all().order_by('enrollment_no')
    if course_id:
...
def faculty_create_exam(request):
    ...
    return Response({'error': 'Exam creation is disabled'}, status=status.HTTP_400_BAD_REQUEST)
...
def faculty_generate_questions(request):
    ...
    return Response({'error': 'Question generation is disabled'}, status=status.HTTP_400_BAD_REQUEST)
...
def faculty_profile(request):
    ...
    serializer = FacultyProfileUpdateSerializer(faculty, data=request.data, partial=True)
    serializer.is_valid(raise_exception=True)
    serializer.save()
    return Response(FacultySerializer(faculty).data)
...
def admin_create_user(request):
    ...
    try:
        if role == 'student':
            ...
            if not course_id:
                return Response({'error': 'course_id is required for student'}, status=status.HTTP_400_BAD_REQUEST)
            course = Course.objects.get(course_id=course_id)
            
            # Map course to prefix
            prefixes = {
                'MCA': '23032432', 'BCA': '23032431', 'BSC-IT': '23032433',
                'BTECH-CSE': '23032434', 'BSC-AIML': '23032435', 'MSC-CYBER': '23032436', 'BTECH-IT': '23032437'
            }
            prefix = prefixes.get(course.code, '23032430')
            
            # Find max existing student with this prefix
            existing = Student.objects.filter(enrollment_no__startswith=prefix).values_list('enrollment_no', flat=True)
            if existing:
                try:
            ...
            user = User.objects.create_user(email=login_email, password=password, role='student')
            ...
            if User.objects.filter(email=email).exists():
                return Response({'error': f'A user with email {email} already exists. Please use a different email.'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            email = provided_email
            if not email:
                return Response({'error': 'Email required for admin'}, status=status.HTTP_400_BAD_REQUEST)
...
GU Admin Team"""
        try:
            send_mail(subj, message, settings.EMAIL_HOST_USER, [provided_email or login_email], fail_silently=True)
        ...
        return Response({'error': f'A user with this email or ID already exists. Details: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
    except Course.DoesNotExist:
        return Response({'error': 'Selected course not found in the system.'}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
...
def admin_analytics(request):
    ...
    enrollment_by_course = Student.objects.values('course__code', 'course__name').annotate(
...
def admin_students(request):
    ...
    course_id = request.query_params.get('course_id')
    students = Student.objects.all().select_related('course', 'user').order_by('-created_at')
    ...
    for s in students:
        ...
        s_data['course_name'] = s.course.name if s.course else 'Pending'
...
def admin_faculty(request):
    ...
    faculty = Faculty.objects.all().select_related('user').prefetch_related('subjects').order_by('-created_at')
...
def admin_assign_subjects(request, faculty_id):
    ...
    return Response({
        ...
        'subjects': [{'subject_id': str(s.subject_id), 'code': s.code, 'name': s.name, 'course_name': s.course.name} for s in subjects.select_related('course')],
    })
...
def admin_holidays(request):
    ...
    if request.method == 'POST':
        ...
        if not serializer.is_valid():
            return Response({'error': 'Invalid holiday data', 'details': serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
        
        try:
            holiday = serializer.save()
        except Exception as e:
            return Response({'error': f'Failed to save holiday: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

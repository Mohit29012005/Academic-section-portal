/* ============================================================
   AI powered Career Guidance system — app.js  (All 5 Features + LLM Integration)
   ============================================================ */
...
async function analyze() {
  ...
  try {
    ...
    if (resumeFile || jobFile) {
      ...
      if (jobFile) fd.append('jobDescription', jobFile);
      if (!resumeFile && resumeText) fd.append('resumeText', resumeText);
      if (!jobFile && jobText) fd.append('jobDescriptionText', jobText);
      const res = await fetch('/api/analyze', { method: 'POST', body: fd });
      response = await res.json().catch(() => ({ error: 'Invalid server response' }));
      if (!res.ok) throw new Error(response.error || 'Analysis failed');
    } else {
      response = await fetchAPI('/api/analyze', { resumeText, jobDescription: jobText });
    }
